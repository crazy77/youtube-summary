...
const shortsReelLinkSelectors = 'a#endpoint[href*="/shorts/"], .reel-player-overlay-action a[href*="/shorts/"]'
...
function getElementInfo(targetElement) {
  ...
  if (targetElement.matches('ytd-reel-video-renderer')) {
    ...
    targetElement.matches('ytd-video-renderer, ytd-compact-video-renderer, ytd-grid-video-renderer, ytd-playlist-panel-video-renderer')
  ) {
    elementType = 'standardVideo'
    baseElement = targetElement
...
function getVideoUrlFromElement(baseElement, elementType) {
  ...
  if (elementType === 'watchTitle') {
    ...
    try {
      ...
      const endpointData = dataHost?.__data?.navigationEndpoint || dataHost?.__data?.data?.navigationEndpoint || dataHost?.__data?.endpoint
...
function findButtonPlacement(baseElement, elementType) {
  if (!baseElement || elementType === 'unknown') return { container: null, insertionPoint: null }
  ...
  if (elementType === 'watchTitle') {
    container =
      baseElement.closest('#title, #info-contents > ytd-video-primary-info-renderer > #container > #info > #info-text') ||
    ...
    container =
      baseElement.querySelector('#metadata-line, #factoids, .reel-video-meta-info') ||
    ...
    if (!container) {
      ...
      if (!container) {
        container = baseElement.querySelector(':scope > #dismissible, #dismissible')
      }
    }
    if (!container) {
...
function addFeloButton(targetElement, useLocationHref = false) {
  ...
  let existingButton = buttonContainer.querySelector(':scope > .felo-search-button')
  if (
    !existingButton &&
    elementType === 'shortsLockup' &&
    insertionPoint &&
    insertionPoint.previousSibling?.matches('div.shortsLockupViewModelHostMetadataSubhead')
  ) {
    if (insertionPoint.previousSibling.nextElementSibling?.classList.contains('felo-search-button')) {
  ...
  button.addEventListener('click', (event) => {
    ...
    const initialElement = clickedButton.closest(potentialElementsSelector + ', #info-contents') // Add watch page container

    if (initialElement) {
      // Re-determine element info based on the container found on click
      const { elementType: clickedElementType, baseElement: clickedBaseElement } =
        initialElement.id === 'info-contents' || initialElement.closest('#info-contents #info-text')
          ? {
              elementType: 'watchTitle',
              baseElement: clickedButton.closest(watchTitleSelectors) || document.querySelector(watchTitleSelectors),
            }
          : getElementInfo(initialElement)

      const urlToOpen = getVideoUrlFromElement(clickedBaseElement, clickedElementType)

      if (urlToOpen) {
        const feloSearchUrl = `https://felo.ai/search?q=${encodeURIComponent(urlToOpen)}`
        window.open(feloSearchUrl, '_blank')
      } else {
        console.warn('[Felo Button] Could not extract valid URL on click from:', clickedBaseElement, 'Type:', clickedElementType)
      }
    } else {
      // Fallback for main watch page if somehow closest fails
      const currentUrl = window.location.href
      if (currentUrl.includes('youtube.com/watch')) {
        console.warn('[Felo Button] Could not find parent element, using current page URL as fallback.')
        const feloSearchUrl = `https://felo.ai/search?q=${encodeURIComponent(currentUrl)}`
        window.open(feloSearchUrl, '_blank')
      } else {
        console.warn('[Felo Button] Could not find parent video element and not on watch page.')
      }
    }
  })
  ...
  try {
    ...
    console.error('[Felo Button] Error inserting button', error, 'Container:', buttonContainer)
  }
}

// --- processPageElements: Iterates and calls addFeloButton ---
function processPageElements() {
  document.querySelectorAll(potentialElementsSelector).forEach((element) => {
    ...
    for (const mutation of mutationsList) {
      if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
        for (const node of mutation.addedNodes) {
          ...
          if (node.nodeType === 1 && (node.matches(potentialElementsSelector) || node.querySelector(potentialElementsSelector))) {
